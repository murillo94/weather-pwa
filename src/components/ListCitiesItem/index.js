...
import { CloudLightning, CloudDrizzle, CloudRain, CloudSnow, Sun, Cloud, TrendingDown, TrendingUp, Thermometer, Wind } from 'react-feather';
...
export default class ListCitiesItem extends Component {
	...
	async getData() {
		try {
			const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${this.props.nameCity}&appid=${key}&units=metric&lang=en`);
	...
	getImageTemperature = () => {
		...
		if(this.state.icon === '50d' || this.state.icon === '50n' || this.state.icon === '02d' || this.state.icon === '02n' || 
			this.state.icon === '03d' || this.state.icon === '03n' || this.state.icon === '04d' || this.state.icon === '04n') {
	...
	render() {
		if(this.state.err) {
			return (
				<Wrapper>
					<Error message={this.state.err} textBtn="Reload" type="list" back={this.actionBack} />
				</Wrapper>
			)
		}
			...
			<Wrapper>
				...
				<ContainerTemperature>
					<SectionTemperature>
						...
						<TemperatureImage>
							{this.getImageTemperature()}
						</TemperatureImage>
					</SectionTemperature>
					<SectionTemperatureMobile>
						<TemperatureInfos tempMin={this.state.temp_min} tempMax={this.state.temp_max} 
						humidity={this.state.humidity} wind={this.state.wind} sizeIcon={15} sizeText={13}
						colorIcon="#c1c1c1" colorText="#7c7c7c" />
					</SectionTemperatureMobile>
				</ContainerTemperature>
			</Wrapper>
		)
